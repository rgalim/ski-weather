...
class SkiAreaServiceTest {
    ...
    private val skiAreaService = SkiAreaService(weatherService, skiAreaWeatherService, scoreService)

    @Test
    fun `should return sorted ski areas when successful forecast response`() =
        runBlocking {
            `when`(weatherService.getWeather()).thenReturn(listOf(WEATHER_FORECAST_SOLDEN, WEATHER_FORECAST_KITZBUHEL))
            `when`(skiAreaWeatherService.convertToSkiAreaWeather(WEATHER_FORECAST_SOLDEN))
                .thenReturn(SKI_AREA_WEATHER_SOLDEN)
            `when`(skiAreaWeatherService.convertToSkiAreaWeather(WEATHER_FORECAST_KITZBUHEL))
    ...
    fun `should throw exception when error forecast response`() =
        runBlocking {
            `when`(weatherService.getWeather()).thenThrow(WeatherApiException("Something went wrong"))
    ...
    fun `should throw exception when failed to convert ski area weather`() =
        runBlocking {
            `when`(weatherService.getWeather()).thenReturn(listOf(WEATHER_FORECAST_SOLDEN))
            `when`(skiAreaWeatherService.convertToSkiAreaWeather(WEATHER_FORECAST_SOLDEN))
